...
use crate::{
  contract::{
    Event, EventWithMetadata, ScribeOptimistic::OpPoked, ScribeOptimisticProvider,
    ScribeOptimisticProviderInstance,
  },
  events_listener::RetryProviderWithSigner,
  metrics,
};
...
impl ScribeEventsProcessor {
  ...
  async fn process_op_poked(&mut self, event: EventWithMetadata, op_poked: OpPoked) -> Result<()> {
    ...
    loop {
      match challenge_attempts {
        0..FLASHBOT_CHALLENGE_RETRY_COUNT => {
          ...
          let contract =
            ScribeOptimisticProviderInstance::new(self.address, self.flashbot_provider.clone());
