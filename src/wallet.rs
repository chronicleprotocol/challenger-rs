    ...
    Ok(
      match (keystore_path, keystore_password, keystore_password_file) {
        ...
        (Some(path), _, Some(password_file)) => {
          ...
          let signer =
            LocalSigner::decrypt_keystore(&path, self.password_from_file(password_file)?)
              .wrap_err_with(|| {
                format!("Failed to decrypt keystore {path:?} with password file {password_file:?}")
              })?;
        ...
        (Some(path), None, None) => {
          ...
          debug!("Using Keystore file from `{path:?}` and password from interactive prompt");
